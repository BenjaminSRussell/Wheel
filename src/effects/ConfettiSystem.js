...
import { cryptoRandomFloatRange, cryptoRandomSignedRange, cryptoRandomArrayElement } from '../utils/crypto.js';
...
export class ConfettiSystem {
  ...
  _createBurst() {
    for (let i = 0; i < CONFETTI_CONFIG.particleCount; i++) {
      const geometry = new THREE.BoxGeometry(CONFETTI_CONFIG.particleSize, CONFETTI_CONFIG.particleSize, CONFETTI_CONFIG.particleSize);
      ...
      confetti.position.set(
        ...
        CONFETTI_CONFIG.initialPosition.yMin + cryptoRandomFloatRange(0, CONFETTI_CONFIG.initialPosition.yMax - CONFETTI_CONFIG.initialPosition.yMin),
      ...
      const baseTurbulence = CONFETTI_CONFIG.physics.turbulence * (0.1 + cryptoRandomFloatRange(0, 1.8));
      
      confetti.userData = {
        velocity: {
          x: cryptoRandomSignedRange(CONFETTI_CONFIG.velocity.xRange) * (0.5 + cryptoRandomFloatRange(0, 1)),
          y: cryptoRandomFloatRange(CONFETTI_CONFIG.velocity.yMin, CONFETTI_CONFIG.velocity.yMax) * (0.3 + cryptoRandomFloatRange(0, 1.4)),
          z: cryptoRandomSignedRange(CONFETTI_CONFIG.velocity.zRange) * (0.5 + cryptoRandomFloatRange(0, 1)),
        },
        rotation: {
          x: cryptoRandomSignedRange(CONFETTI_CONFIG.rotation.range) * (0.2 + cryptoRandomFloatRange(0, 1.6)),
          y: cryptoRandomSignedRange(CONFETTI_CONFIG.rotation.range) * (0.2 + cryptoRandomFloatRange(0, 1.6)),
          z: cryptoRandomSignedRange(CONFETTI_CONFIG.rotation.range) * (0.2 + cryptoRandomFloatRange(0, 1.6)),
        },
        gravity: baseGravity,
  ...
  _applyMouseInteraction(confetti, userData) {
    ...
    if (distance < CONFETTI_CONFIG.physics.mouseInteraction.radius) {
      const force = CONFETTI_CONFIG.physics.mouseInteraction.strength * (1 - distance / CONFETTI_CONFIG.physics.mouseInteraction.radius);
  ...
  update() {
    ...
    this.confettiPieces = this.confettiPieces.filter((confetti) => {
      ...
      const floatZ = Math.cos(particleAge * userData.floatFrequency * 1.3) * userData.floatAmplitude * 0.7;
      ...
      const windX = (Math.sin(particleAge * 0.2) * userData.windEffect + 
                    Math.sin(particleAge * 0.5) * userData.windEffect * 0.3 +
                    Math.sin(particleAge * 0.8) * userData.windEffect * 0.1) * userData.windSensitivity;
      const windZ = (Math.cos(particleAge * 0.3) * userData.windEffect + 
                    Math.cos(particleAge * 0.6) * userData.windEffect * 0.2 +
                    Math.cos(particleAge * 0.9) * userData.windEffect * 0.1) * userData.windSensitivity;
      
      userData.velocity.x += windX;
      userData.velocity.z += windZ;
      
      const turbulenceIntensity = userData.turbulence * userData.turbulenceSensitivity * 
                                 (0.3 + Math.sin(particleAge * 1.5) * 0.4);
      userData.velocity.x += (Math.random() - 0.5) * turbulenceIntensity;
      userData.velocity.y += (Math.random() - 0.5) * turbulenceIntensity * 0.3;
      userData.velocity.z += (Math.random() - 0.5) * turbulenceIntensity;
      ...
      confetti.rotation.y += userData.rotation.y * (0.5 + Math.cos(particleAge * userData.floatFrequency * 0.7) * 0.3);
      confetti.rotation.z += userData.rotation.z * (0.7 + Math.sin(particleAge * userData.floatFrequency * 1.2) * 0.3);
    ...
    if (elapsedSeconds >= CONFETTI_CONFIG.streamDuration / 1000 && this.confettiPieces.length === 0) {
      this.isActive = false;
    }
  }
}
